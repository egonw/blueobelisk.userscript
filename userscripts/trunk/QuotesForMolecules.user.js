...
function main() {
  ...
  {
    get_INCHI_list("http://cb.openmolecules.net/api_inchi.php?type=inchi&ids_only=1", "chemical_blogspace_ID_list");
...
function get_citations_cb(doi, textarea) {
  //GM_log("param: " + doi);
  url = "http://cb.openmolecules.net/api.php?type=post&citing_cbid="+doi+"&format=json";
...
function insert_citations_cb(obj, doi, textarea) {
  ...
  newanchor.setAttribute("href","http://cb.openmolecules.net/inchi.php?id="+doi);
